...
function loadcalendar() {
	...
	s += '<tr align="center" id="calendar_week"><td><a href="javascript:;" onclick="refreshcalendar(yy, mm-1)" title="上一月">&laquo;</a></td><td colspan="5" style="text-align: center"><a href="javascript:;" id="year" onclick="showdiv(this,\'year\');doane(event)" class="dropmenu" title="选择年"></a>&nbsp; - &nbsp;<a id="month" class="dropmenu" title="选择月" href="javascript:;" onclick="showdiv(this,\'month\');doane(event)"></a></td><td><A href="javascript:;" onclick="refreshcalendar(yy, mm+1)" title="下一月">&raquo;</A></td></tr>';

	s += '<tr id="calendar_header"><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td><td>日</td></tr>';
	...
	for ( var k = 2020; k >= 1931; k--) {
		...
		s += '<a href="javascript:;" onclick="refreshcalendar(' + k

				+ ', mm);$(\'calendar_year\').style.display = \'none\'"><span'

				+ (today.getFullYear() == k ? ' class="calendar_today"' : '')
	...
	for ( var k = 1; k <= 12; k++) {
		...
		s += '<a href="javascript:;" onclick="refreshcalendar(yy, ' + (k - 1)

				+ ');$(\'calendar_month\').style.display = \'none\'"><span'

				+ (today.getMonth() + 1 == k ? ' class="calendar_today"' : '')
	...
	$('append_parent_date').appendChild(div);
...
function initclosecalendar() {
	...
	while (aim.parentNode != document.body) {
		...
		if (aim.parentNode.id == 'append_parent_date') {
...
function showcalendar(event, controlid1, addtime1, startdate1, enddate1) {
	...
	if (lastcheckedyear != false) {
		...
		$('calendar_year_' + lastcheckedyear).setAttribute('class', 'calendar_default');

		$('calendar_year_' + today.getFullYear()).setAttribute('class','calendar_today');

	}

	if (lastcheckedmonth != false) {

		$('calendar_month_' + lastcheckedmonth).setAttribute('class','calendar_default');

		$('calendar_month_' + (today.getMonth() + 1)).setAttribute('class', 'calendar_today');

	}

	$('calendar_year_' + currday.getFullYear()).setAttribute('class', 'calendar_checked');

	$('calendar_month_' + (currday.getMonth() + 1)).setAttribute('class', 'calendar_checked');
 ...
 * end ---88888888888888888888888888888888888888888888888888888888888888888888888888888888888888
...
function refreshcalendar(y, m) {
	...
	while (x.getMonth() == mm) {
		...
		dd.innerHTML = ( '<a href="javascript:;" onclick="settime(' + d
		...
		if (x.getTime() < today.getTime()
			...
			dd.setAttribute('class', 'calendar_expire');

		} else {

			dd.setAttribute('class', 'calendar_default');
		...
		if (x.getFullYear() == today.getFullYear()
			...
			dd.setAttribute('class', 'calendar_today');

			dd.setAttribute('title' , '标题');
		...
		if (x.getFullYear() == currday.getFullYear()
			...
			dd.setAttribute('class', 'calendar_checked');
