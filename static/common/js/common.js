...
function ajaxmenu(ctrlObj, timeout, cache, duration, pos, recall, idclass,
	...
	if (menu) {
		...
		var url = (!isUndefined(ctrlObj.attributes['shref']) ? ctrlObj.attributes['shref'].value
				: (!isUndefined(ctrlObj.href) ? ctrlObj.href
...
function showloading(display, waiting) {
	...
	var waiting = waiting ? waiting : '加载中……';
...
function onShow(msg, right, t) {
	// 'confirm', 'notice', 'info', 'right'
	showDialog(msg, 'info', 'onshow', right, '', '', '', '', '', t);
}
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt,
	...
	if (closetime) {
		leftmsg = closetime + '秒钟后自动关闭';
		showDialogST = setTimeout(closefunc, closetime * 1000);
		showconfirm = 0;
	}
	locationtime = isUndefined(locationtime) ? '' : locationtime;
	if (locationtime) {
		leftmsg = locationtime + '分钟';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : '确定';
	...
	{
		var s = hidedom
		+ '<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">';
		s +='<div class="' + func + '"><p>';		
	}else{		
	...
	if (mode == 'info') {
		...
		s += '<p class="o pns">'
				...
				+ (showconfirm ? '<button id="fwin_dialog_submit" value="true" class="pn pnc"><strong>'
...
function showWindow(k, url, mode, cache, menuv) {
	...
	if (!menuObj) {
		...
		if (mode == 'html') {
			...
			smiliestype = '<ul class="tb tb_s cl" id="e_image_ctrl" style="margin-top: 0px; margin-bottom: 0px; cursor: move;">';
			smiliestype += '<li class="z"><h3 class="flb" style="cursor: move;"><em>图片管理</em>	</h3></li><li class="y">';
			smiliestype += '<span class="flbc" onclick="hideWindow(\'' + k
					+ '\')">关闭</span></li></ul>';
			smiliestype += '<iframe frameborder="0"	src="' + url
					+ '" style="height: 300px; width: 580px;"></iframe>';
			$('fwin_content_' + k).innerHTML = smiliestype;
			initMenu();
			show();
		} else {
			fetchContent();
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if (msg !== '') {
		showDialog(msg, 'alert', '文件加载中', null, true, null, '', '', '', 3);
	}
}
...
// 全选
function checkAllCK(type, form, value, checkall, changestyle) {
	var checkall = checkall ? checkall : 'select_all2';
	for ( var i = 0; i < form.elements.length; i++) {
		var e = form.elements[i];
		if (type == 'option' && e.type == 'radio' && e.value == value
				&& e.disabled != true) {
			e.checked = true;
		} else if (type == 'value' && e.type == 'checkbox'
				&& e.getAttribute('chkvalue') == value) {
			e.checked = form.elements[checkall].checked;
			if (changestyle) {
				multiupdate(e);
			}
		} else if (type == 'prefix' && e.name && e.name != checkall) {
			e.checked = form.elements[checkall].checked;
			if (changestyle) {
				...
				if (e.parentNode.parentNode
						&& e.parentNode.parentNode.tagName.toLowerCase() == 'div') {
					e.parentNode.parentNode.className = e.checked ? 'item checked'
							: 'item';
				}
			}
		}
...
function checkall(type, form, value, checkall) {
	...
	for ( var i = 0; i < form.elements.length; i++) {
		var e = form.elements[i];
		if (type == 'prefix' && e.name && e.name != checkall && e.name != "del") {
			e.checked = form.elements[checkall].checked;
		}
	}
}
